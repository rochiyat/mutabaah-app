  ...
  const handleRecordUpdate = async (activityId: number, completed: number) => {
    try {
      ...
      const existingRecord = records.find(
        (r) => r.activityId === activityId && r.date.startsWith(today)
      );
    ...
    <div className="today-checklist">
      ...
      {activities.map((activity) => {
        ...
        const todayRecord = records.find(
          (r) => r.activityId === activity.id && r.date.startsWith(today)
        );
        ...
        return (
          <div key={activity.id} className="checklist-item">
            ...
            <input
              ...
              onChange={(e) =>
                handleRecordUpdate(activity.id, parseInt(e.target.value))
              }
              max={activity.target}
            />
            <span>
