...
export const activityAPI = {
  ...
  createActivity: (name: string, category: string, target?: number, unit?: string) =>
...
export const recordAPI = {
  getRecords: (activityId?: number, startDate?: string, endDate?: string) =>
    api.get<Record[]>('/records', { params: { activityId, startDate, endDate } }),
  createRecord: (activityId: number, completed: number, notes?: string, date?: string) =>
    api.post<Record>('/records', { activityId, completed, notes, date }),
  updateRecord: (id: number, data: any) =>
    api.patch<Record>(`/records/${id}`, data),
